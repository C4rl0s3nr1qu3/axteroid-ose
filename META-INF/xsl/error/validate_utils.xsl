...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	exclude-result-prefixes="dp dyn regexp date func" version="1.0">
...
<!--   <xsl:include href="../../../VALI/commons/StringTemplates.xsl"/> -->
  ...
  <xsl:include href="../StringTemplates.xsl" dp:ignore-multiple="yes"/>
          ...
          <xsl:choose>
            ...
            <xsl:otherwise>
              <xsl:call-template name="addWarning">
                ...
                <xsl:if test="not(fn:matches($node, $regexp,'!'))">
                    <xsl:choose>
                        <xsl:when test="$isError">
                            <xsl:call-template name="rejectCall">
        ...
        <xsl:otherwise>
          <xsl:call-template name="addWarning">
            ...
            <xsl:with-param name="warningMessage"
                ...
                <xsl:if test="count($node) &gt;= 1 and not(fn:matches($node, $regexp,'!'))">
                    <xsl:choose>
                        <xsl:when test="$isError">
                            <xsl:call-template name="rejectCall">
                                <xsl:with-param name="errorCode" select="$errorCodeValidate" />
  ...
  <xsl:template name="findElementInCatalog">
	...
	<xsl:variable name="url_catalogo" select="concat('../../../META-INF/xsl/cpe/catalogo/cat_',$catalogo,'.xml')"/> 
	<!-- <xsl:variable name="url_catalogo" select="concat('../../../template/xsl/cpe/catalogo/cat_',$catalogo,'.xml')"/>-->
  ...
  <xsl:template name="findElementInCatalogProperty">
	...
	<xsl:variable name="url_catalogo" select="concat('../../../META-INF/xsl/cpe/catalogo/cat_',$catalogo,'.xml')"/>
	<!-- <xsl:variable name="url_catalogo" select="concat('../../../template/cpe/catalogo/cat_',$catalogo,'.xml')"/>-->

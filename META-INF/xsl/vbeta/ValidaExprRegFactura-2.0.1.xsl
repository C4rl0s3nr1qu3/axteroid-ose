...
<xsl:stylesheet version="2.0" 	
	...
	<xsl:template match="/*">
		...
		<!-- <xsl:variable name="porcentajeIGVVigenteLinea" select="min(cac:InvoiceLine/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory[cac:TaxScheme/cbc:ID[text() = '1000' or text() = '9996']]/cbc:Percent)"/>-->		
				...
				<xsl:for-each select="./cac:AdditionalItemProperty">
					...
					<xsl:if test="($codigoConcepto='7003') or ($codigoConcepto='7004')">							
						...
						<xsl:call-template name="regexpValidateElementIfExistTrue">
							...
							<xsl:with-param name="isError" select="false()"/> 
							<xsl:with-param name="descripcion" 
								select="concat('Error en la linea: 20b): ', position(), ' ', $codigoConcepto, ' ',$codigoConcepto_Value)"/>
						</xsl:call-template>
					</xsl:if>	
...
<!-- 					</xsl:if>			 -->
					...
					<xsl:if test="($codigoConcepto='7013')">					
						<xsl:call-template name="regexpValidateElementIfExist">
							<xsl:with-param name="errorCodeValidate" select="'4280'"/>
							<xsl:with-param name="node" select="$codigoConcepto_Value"/>
							<xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{0,49}$'"/>
							<xsl:with-param name="isError" select="false()"/>
							<xsl:with-param name="descripcion" 
						...
						<xsl:call-template name="regexpValidateElementIfExistTrue">
							...
							<xsl:with-param name="isError" select="false()"/> 
							<xsl:with-param name="descripcion" 
								select="concat('Error en la linea: 32) ', position(), ' ', $codigoConcepto, ' ',$codigoConcepto_Value)"/>
						</xsl:call-template>
					</xsl:if>		
					...
					<xsl:if test="($codigoConcepto='7015')">					
						<xsl:call-template name="regexpValidateElementIfExist">
							...
							<xsl:with-param name="isError" select="false()"/>
							<xsl:with-param name="descripcion" 
								select="concat('Error en la linea: 33) ', position(), ' ', $codigoConcepto, ' ',$codigoConcepto_Value)"/>
						</xsl:call-template>	
					...
					<xsl:if test="($codigoConcepto='7016')">					
						<xsl:call-template name="regexpValidateElementIfExist">
							...
							<xsl:with-param name="isError" select="false()"/>
							<xsl:with-param name="descripcion" 
								select="concat('Error en la linea: 34) ', position(), ' ', $codigoConcepto, ' ',$codigoConcepto_Value)"/>
						</xsl:call-template>	
					</xsl:if>		
					...
					<xsl:for-each select="./cac:UsabilityPeriod">	
						...
						<xsl:if test="$codigoConcepto = '7014'">
							...
							<xsl:call-template name="regexpValidateElementIfExist">
								...
								<xsl:with-param name="isError" select="false()"/>
								<xsl:with-param name="descripcion" 
									select="concat('Error en la linea: 35) ', position(), ' ', $codigoConcepto, ' ', $fechaInicio)"/>
							</xsl:call-template>
							...
							<xsl:call-template name="regexpValidateElementIfExist">
								...
								<xsl:with-param name="isError" select="false()"/>
								<xsl:with-param name="descripcion" 
									select="concat('Error en la linea: 36) ', position(), ' ', $codigoConcepto, ' ', $fechaFin)"/>
							</xsl:call-template>												
						</xsl:if>									
						...
						<xsl:if test="($codigoConcepto='7014')">							
							<xsl:call-template name="regexpValidateElementIfExist">
								...
								<xsl:with-param name="isError" select="false()"/>
								<xsl:with-param name="descripcion" 
									select="concat('Error en la linea: 37) ', position(), ' ', $codigoConcepto, ' ', $fechaInicio)"/>
							</xsl:call-template>
							...
							<xsl:call-template name="regexpValidateElementIfExist">
								...
								<xsl:with-param name="isError" select="false()"/>
								<xsl:with-param name="descripcion" 
									select="concat('Error en la linea: 38) ', position(), ' ', $codigoConcepto, ' ', $fechaFin)"/>
							</xsl:call-template>							
						</xsl:if>						
					</xsl:for-each>							
				</xsl:for-each>				
							...
							<xsl:if test="((($codigoTributoLinea = 1000) and ($montoBase > 0)) or 
								...
								<xsl:if test="not(number($tasaIGVoIVAP) = number($porcentajeIGVVigenteLinea))">	
									<xsl:call-template name="rejectCall">
										...
										<xsl:with-param name="errorMessage"
											select="concat('Error en la linea: ', $numeroLinea, ' | ', $codigoTributoLinea, ' | ', $tasaIGVoIVAP, ' | ', $porcentajeIGVVigenteLinea)"/>
									</xsl:call-template>																																						
								</xsl:if>
							</xsl:if>
			...
			<xsl:if test="($sumMontoBase9996LineaSub &gt; 0)">	
				...
				<xsl:if test="($dif_TotalImporte &lt; -1) or ($dif_TotalImporte &gt; 1)">
					<xsl:call-template name="rejectCall">
						...
						<xsl:with-param name="errorMessage"
							select="concat('Error en la linea: 1) ',$numeroLinea, ' ', $dif_TotalImporte, ' ', $valorVentaLinea, ' ', $totalImporteCalculado,' ', $valorReferencialUnitarioItemOperacionesGratuitas, ' ',$montoDescuentosLinea00, ' ',$montoCargosLinea47, ' ',$cantidadUnidadesLinea)"/>
					</xsl:call-template>					
				</xsl:if>	
			</xsl:if>			
			...
			<xsl:if test="not($sumMontoBase9996LineaSub &gt; 0)">	
				...
				<xsl:if test="($dif_TotalImporte &lt; -1) or ($dif_TotalImporte &gt; 1)">
					<xsl:call-template name="rejectCall">
						<xsl:with-param name="errorCode" select="'3271'" />
						<xsl:with-param name="errorMessage"
							select="concat('Error en la linea: 2) ',$numeroLinea, ' ', $dif_TotalImporte, ' ', $valorVentaLinea, ' ', $totalImporteCalculado,' ', $valorUnitarioItem, ' ',$montoDescuentosLinea00, ' ',$montoCargosLinea47, ' ',$cantidadUnidadesLinea)"/>
					</xsl:call-template>					
				</xsl:if>			
			</xsl:if>							
	    ...
	    <xsl:if test="cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '1000']]">
			...
			<xsl:if test="($dif_TotalImporte &lt; -1) or ($dif_TotalImporte &gt; 1)">
				<xsl:call-template name="rejectCall">
					...
					<xsl:with-param name="errorMessage"
						select="concat('Error en la linea: ', $dif_TotalImporte, ' ', $sumatoriaIGV, ' ', $totalImporteCalculado, ' ', $totalBaseIGVxLinea, ' ', $sumatoriaDescuentosGlobales, ' ', $sumatoriaCargosGobales_49, ' ',$sumatoriaAnticipos_20, ' ',$porcentajeIGVVigenteLineaDecimal)"/>
				</xsl:call-template>
			</xsl:if>
	    </xsl:if>
		...
		<xsl:if test="($sumatoriaTotalPrecioVentaSubTotal) and ($countCodigoTributo1016 = 0)">	
			...
			<xsl:if test="($dif_TotalImporte &lt; -1) or ($dif_TotalImporte &gt; 1)">
				<xsl:call-template name="rejectCall">
					...
					<xsl:with-param name="errorMessage"
						select="concat('Error en la linea: a) ', $dif_TotalImporte, ' ', $sumatoriaTotalPrecioVentaSubTotal, ' ', $totalImporteCalculado, ' ', $totalImpuestoCalculado, ' ', $sumaTotalPrecioVenta, ' ', $porcentajeIGVVigenteLineaDecimal)"/>
				</xsl:call-template>			
			</xsl:if>			
		</xsl:if>	   		 
...
<!-- 				</xsl:call-template> -->
		...
		<xsl:call-template name="regexpValidateElementIfExist">
			...
			<xsl:with-param name="regexp" 
				select="'^(0101|0112|0200|0201|0202|0203|0204|0205|0206|0207|0208|0301|0302|0401|1001|1002|1003|1004|2001|2002|2100|2101|2102|2103|2104|2106)$'"/>
			<xsl:with-param name="descripcion" 
				select="concat('Error en la linea: ', position(), ' ', $tipoOperacion)"/>
		</xsl:call-template>

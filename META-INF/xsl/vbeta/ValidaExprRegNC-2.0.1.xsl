...
<xsl:stylesheet  version="2.0" 
	...
	<xsl:template match="/*">
		...
		<xsl:variable name="porcentajeIGVVigenteLinea" select="max(cac:CreditNoteLine/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory[cac:TaxScheme/cbc:ID[text() = '1000' or text() = '9996']]/cbc:Percent)"/>
							...
							<xsl:if test="($codigoTributo='9999') and ($tipoDocumentoDocumentoModificaGlobal = '01')">
								...
								<xsl:if test="($dif_TotalValorVenta &lt; -1) or ($dif_TotalValorVenta &gt; 1)">	
									<xsl:call-template name="rejectCall">
										...
										<xsl:with-param name="errorMessage" 
											select="concat('Error en la linea: ', $dif_TotalValorVenta, ' ', $totalValorVentaItem, ' ', $sumMontoBase9999Linea)"/>
									</xsl:call-template>									
					...
					<xsl:for-each select="./cac:TaxCategory">
						...
						<xsl:if test="($codigoTipoNotaCredito = '12') and not($afectacionIGVLinea = '17')">	
								<xsl:call-template name="rejectCall">
									...
									<xsl:with-param name="errorMessage"
										select="concat('Error en la linea: ', $numeroLinea, ') ', $codigoTipoNotaCredito, ' ', $afectacionIGVLinea)"/>
								</xsl:call-template>
						</xsl:if>	
							-->

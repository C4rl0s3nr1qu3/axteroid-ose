...
<!-- 						<xsl:with-param name="regexp" select="'^([B][A-Z0-9]{3})-(?!0+$)([0-9]{1,8})$|^[0-9]{1,4}-[0-9]{1,8}$'"/> -->
			...
			<xsl:for-each select="./cac:TaxTotal">	
				...
				<!-- ////  -->
				<!--
 				<xsl:if test="($codigoTributo = '1000') and ($totalIGV &gt; 0)"> 
 					<xsl:variable name="sumIGV_Calc" select="(($sumTVVOGravadas + $sumatoriaISC) * number($porcentajeIGV))" /> 
 					...
 					<xsl:if test="($difSumIGV &lt; -5) or ($difSumIGV &gt; 5)"> 
 						<xsl:call-template name="addWarning"> 
 							...
 							<xsl:with-param name="warningMessage" 
 								select="concat('Error en la linea: ', $numeroLinea, '-', $difSumIGV,'-', $sumIGV,'-', $sumIGV_Calc,'-', $sumTVVOGravadas,'-', $sumatoriaISC, '-', $porcentajeIGV)"/> 
 						</xsl:call-template> 
 					</xsl:if>	 
 				</xsl:if>	 
 				-->

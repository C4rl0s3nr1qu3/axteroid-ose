	...
	<xsl:template match="/*">
		...
		<xsl:for-each select="cac:InvoiceLine">
			...
			<xsl:for-each select="./cac:ItemPriceExtension">
				...
				<xsl:if test="($indicadorTipoComisionLinea = '1')">
				   	<xsl:variable name="dif_importeTotal" select="$importeTotal - ($sumTotalComisionesLinea + $sumTotalIGVLinea)" />
					<xsl:if test="($dif_importeTotal &lt; -1) or ($dif_importeTotal &gt; 1)">
						<xsl:call-template name="addWarning">
							<xsl:with-param name="warningCode" select="'4348'" />
							<xsl:with-param name="warningMessage"
								select="concat('Error en la linea: 1) ', $numeroItem, ' ', $dif_importeTotal, ' ', $importeTotal, ' ', $sumTotalComisionesLinea, ' ', $sumTotalIGVLinea)"/>
						</xsl:call-template>
					</xsl:if>				
				</xsl:if>
				
				<xsl:if test="($indicadorTipoComisionLinea = '2')">								
				   	<xsl:variable name="dif_importeTotal" select="$importeTotal - ($sumTotalIGVCargoLinea + $sumTotalIGVDescuentoLinea)" />
					<xsl:if test="($dif_importeTotal &lt; -1) or ($dif_importeTotal &gt; 1)">
						<xsl:call-template name="addWarning">
							<xsl:with-param name="warningCode" select="'4348'" />
							<xsl:with-param name="warningMessage"
								select="concat('Error en la linea: 2) ', $numeroItem, ' ', $dif_importeTotal, ' ', $importeTotal, ' ', $sumTotalIGVCargoLinea, ' ', $sumTotalIGVDescuentoLinea)"/>
						</xsl:call-template>
					</xsl:if>				
				</xsl:if>				

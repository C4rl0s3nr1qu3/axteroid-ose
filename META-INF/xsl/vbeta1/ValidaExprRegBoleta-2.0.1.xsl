		...
		<xsl:for-each select="cac:AccountingSupplierParty">
			...
			<xsl:for-each select="./cac:Party">
				<xsl:for-each select="./cac:PartyIdentification">
					<!-- 9. Número de RUC -->
					<xsl:variable name="emisorNumeroDocumento"
						select="./cbc:ID" />
					<!-- 9. Tipo de documento de identidad del emisor -->
					<xsl:variable name="emisorTipoDocumento"
						select="./cbc:ID/@schemeID" />
					<xsl:variable name="emisorNumeroDocumento_SchemeName"
						select="./cbc:ID/@schemeName" />
					<xsl:variable
						name="emisorNumeroDocumento_SchemeAgencyName"
						select="./cbc:ID/@schemeAgencyName" />
					<xsl:variable name="emisorNumeroDocumento_SchemeURI"
				...
				<xsl:if test="$tipoOperacion = '2104'">					
					<xsl:if test="($countCodigoConcepto7015For = 0)">
						<xsl:call-template name="rejectCall">
							...
							<xsl:with-param name="errorMessage"
								select="concat('Error countcodigoConcepto: ', $countCodigoConcepto7015For)"/>
						</xsl:call-template>							
					</xsl:if>						
		       </xsl:if>
				...
				<xsl:for-each select="./cac:AdditionalItemProperty">
					...
					<xsl:if test="($codigoConcepto='3050') or ($codigoConcepto='3051') or ($codigoConcepto='3052') or 
						...
						<xsl:call-template name="existValidateElementNotExist">
							...
							<xsl:with-param name="descripcion"
								select="concat('Error en la linea: ', position(), ' ', $codigoConcepto_Value)" />
						</xsl:call-template>
					</xsl:if>
...
<!-- 							<xsl:with-param name="regexp" select="'^(?!\s*$)[^\s].{0,14}$'"/> -->
					...
					<xsl:if test="($codigoConcepto='7004')">					
						<xsl:call-template name="regexpValidateElementIfExist">
							...
							<xsl:with-param name="isError" select="false()"/>
							<xsl:with-param name="descripcion" 
								select="concat('Error en la linea (s): ', position(), ' ', $codigoConcepto, ' ',$codigoConcepto_Value)"/>
						</xsl:call-template>	
						...
						<xsl:call-template name="regexpValidateElementIfExistTrue">
							...
							<xsl:with-param name="isError" select="false()"/> 
							<xsl:with-param name="descripcion" 
								select="concat('Error en la linea (t): ', position(), ' ', $codigoConcepto, ' ',$codigoConcepto_Value)"/>
						</xsl:call-template>					
					</xsl:if>								
					...
					<xsl:if test="($codigoConcepto='7017')">
						<xsl:call-template name="regexpValidateElementIfExist">
							...
							<xsl:with-param name="isError" select="false()" />
							<xsl:with-param name="descripcion"
								select="concat('Error en la linea: 8) ', position(), ' ', $codigoConcepto_Value)" />
						</xsl:call-template>
					</xsl:if>
...
<!-- 						</xsl:if> -->
	    ...
	    <xsl:if test="(cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '1000']]) and 
			...
			<xsl:if test="($dif_TotalImporte &lt; -1) or ($dif_TotalImporte &gt; 1)">
				<xsl:call-template name="addWarning">
					...
					<xsl:with-param name="warningMessage"
						select="concat('Error en la linea 1): ', $dif_TotalImporte, ' ', $sumatoriaIGV, ' ', $totalImporteCalculado, ' ', $totalBaseIGVxLinea, ' ', $sumatoriaDescuentosGlobales, ' ', $sumatoriaCargosGobales_49, ' ', $sumatoriaAnticipos_20)"/>
				</xsl:call-template>
			</xsl:if>
	    </xsl:if>
	    ...
	    <xsl:if test="(cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '1000']]) and 
			...
			<xsl:variable name="totalImporteCalculado" select="($totalBaseIGVxLinea - $sumatoriaDescuentosGlobales + $sumatoriaCargosGobales_49) * 0.18"/>			
			<xsl:variable name="dif_TotalImporte" select="$sumatoriaIGV - $totalImporteCalculado" />
			<xsl:if test="($dif_TotalImporte &lt; -1) or ($dif_TotalImporte &gt; 1)">
				<xsl:call-template name="addWarning">
					<xsl:with-param name="warningCode" select="'4290'" />
					<xsl:with-param name="warningMessage"
						select="concat('Error en la linea 2): ', $dif_TotalImporte, ' ', $sumatoriaIGV, ' ', $totalImporteCalculado, ' ', $totalBaseIGVxLinea, ' ', $sumatoriaDescuentosGlobales, ' ', $sumatoriaCargosGobales_49)"/>
				</xsl:call-template>
			</xsl:if>
	    </xsl:if>
...
<!-- 			<xsl:variable name="sumaTotalPrecioVenta" select="$sumatoriaBaseIGV + $sumatoriaISC + $sumatoriaOtrosTributos + $sumatoriaICBPER"/>		 -->
			...
			<xsl:if test="($dif_TotalImporte &lt; -1) or ($dif_TotalImporte &gt; 1)">
				<xsl:call-template name="addWarning">
					...
					<xsl:with-param name="warningMessage"
						select="concat('Error en la linea: a) ', $dif_TotalImporte, ' | ', $sumatoriaTotalPrecioVentaSubTotal, ' | ', $totalImporteCalculado, ' | ', $totalImpuestoCalculado, ' | ', $sumaTotalPrecioVenta, ' | ',$totalBaseIGVxLinea, ' | ',$sumatoriaDescuentosGlobales_02, ' | ',$sumatoriaCargosGobales_49)"/>
				</xsl:call-template>
			</xsl:if>
<!-- 							<xsl:call-template name="addWarning"> -->
...
<!-- 				</xsl:call-template> -->
		...
		<xsl:if test="($sumatoriaTotalPrecioVentaSubTotal) and ($countCodigoTributo1016 &gt; 0)">
<!-- 			<xsl:variable name="sumaTotalPrecioVenta" select="$sumatoriaBaseIGV + $sumatoriaOtrosTributos + $sumatoriaICBPER"/>	 -->
			...
			<xsl:variable name="totalPrecioVenta">	
				<xsl:choose>
    				<xsl:when test="count(./cbc:TaxInclusiveAmount) &gt; 0">
    					<xsl:value-of select="sum(./cbc:TaxInclusiveAmount)"/></xsl:when>
    				<xsl:otherwise>0</xsl:otherwise>
  				</xsl:choose>       
			</xsl:variable>				
						...
						<xsl:if test="($tipoComprobanteRealizoAnticipo='02')">
							<xsl:call-template
								...
								<xsl:with-param name="errorCodeValidate"
									select="'2521'" />
								<xsl:with-param name="node"
									select="$serieNumeroDocumentoRealizoAnticipo" />
								<xsl:with-param name="regexp"
...
<!-- 							<xsl:with-param name="regexp" select="'^[\w\s].{1,30}$'" /> -->
		...
		<xsl:for-each select="cac:PaymentTerms">
			...
			<xsl:if test="($indicadorPaymentTerms='Detraccion')">
				...
				<xsl:if
					test="$tipoOperacion='1003' and not($codigoBienServicio='026')">
					<xsl:call-template name="rejectCall">
						<xsl:with-param name="errorCode" select="'3129'" />
						<xsl:with-param name="errorMessage"
							select="concat('Error en la linea: ', $indicadorPaymentTerms, ' ', $tipoOperacion, ' ', $codigoBienServicio)" />

...
<!-- FIN: AXTEROID -->	
	...
	<xsl:template match="/*">
		...
		<!-- Opcional -->
		<!-- 13. Dirección del lugar en el que se entrega el bien o se presta el servicio -->
		...
		<!-- Opcional -->
		<!-- 16. Apellidos y nombres, denominación o razón social -->
...
<!-- 		<xsl:variable name="additionalMonetaryTotalID" select="$additionalMonetaryTotal/cbc:ID" /> -->
		...
		<xsl:call-template name="existAndRegexpValidateElement">
			...
			<xsl:with-param name="regexp" select="'^(01|14)$'" />
			<xsl:with-param name="descripcion" 
		...
		<!-- Datos del ciente o receptor -->	
		<!-- 15. Valida que el tipo de documento del adquiriente exista y sea solo uno -->		
		...
		<!-- 15. Valida que el tipo de documento del adquiriente exista -->
		<xsl:call-template name="existValidateElementIfExist">
		...
		<xsl:if test="$tipoOperacion='13'">		
			<xsl:if
				test="not($clienteTipoDocumento='1' or $clienteTipoDocumento='6')">
				<xsl:call-template name="rejectCall">
					<xsl:with-param name="errorCode" select="'2800'" />
					<xsl:with-param name="errorMessage"
						select="concat('Error en la linea: ', $tipoOperacion, ' ', $clienteTipoDocumento)"/>
				</xsl:call-template>
			</xsl:if>
			<xsl:if
				test="$clienteTipoDocumento='1' and not(fn:matches($clienteNumeroDocumento,'^[0-9]{8}$'))">
				<xsl:call-template name="rejectCall">
					<xsl:with-param name="errorCode" select="'2801'" />
					<xsl:with-param name="errorMessage"
						select="concat('Error en la linea: ', $tipoOperacion, ' ', $clienteTipoDocumento)"/>
				</xsl:call-template>
			</xsl:if>
		</xsl:if>
		<xsl:if test="not($tipoOperacion='13' or $tipoOperacion='02')">
			<xsl:if test="$clienteTipoDocumento!='6'">
				<xsl:call-template name="addWarning">
					<xsl:with-param name="warningCode" select="'2016'" />
					<xsl:with-param name="warningMessage"
						select="concat('Error en la linea: ', $tipoOperacion, ' ', $clienteTipoDocumento)"/>
				</xsl:call-template>
			</xsl:if>		
		...
		<!-- 19. Número de la guía de remisión relacionada -->
		...
		<!-- 20. Número de otro documento relacionado -->
			...
			<xsl:if test="$numeroOtroDocumentoRelacionada">
				<xsl:if
					...
					<xsl:call-template name="addWarning">
						...
						<xsl:with-param name="warningMessage"
							select="concat('Error en la linea: ',' ',$numeroOtroDocumentoRelacionada, $tipoOtroDocumentoRelacionada)"/>
					</xsl:call-template>
				</xsl:if>			
			</xsl:if>
		...
		<xsl:for-each select="cac:InvoiceLine">
			...
			<!-- 27. Descripción detallada del servicio prestado, bien vendido o cedido en uso, indicando las características. -->		
<!-- 			<xsl:variable name="descripcionDetalladaServicioPrestado" select="./cac:Item/cbc:Description"/> -->
			<!-- 28. Valor unitario por ítem -->		
			<xsl:variable name="valorUnitarioItem" select="./cac:Price/cbc:PriceAmount"/>	
			<!-- 28. Moneda de Valor unitario por ítem -->		
			...
			<!-- 23. Cantidad de unidades por item -->	
			<xsl:call-template name="existValidateElementIfExist">
...
<!-- 				<xsl:call-template name="addWarning"> -->
<!-- 					<xsl:with-param name="warningCode" select="'2027'" /> -->
<!-- 					<xsl:with-param name="warningMessage" -->
<!-- 						select="concat('Error en la linea: ', position(), ' ')" /> -->
<!-- 				</xsl:call-template> -->
<!-- 			</xsl:if> -->
...
<!-- 			</xsl:call-template>			 -->
			...
			<!-- 29. Precio de venta unitario por item -->
			...
			<xsl:for-each select="./cac:PricingReference/cac:AlternativeConditionPrice">
				...
				<!-- 29. Moneda Precio de venta unitario por item -->		
			...
			<xsl:for-each select="./cac:TaxTotal">
				...
				<!-- 32. Monto de ISC de la línea -->		
				<xsl:variable name="montoISCLinea" select="./cbc:TaxAmount"/>	
		...
		<!-- 37. El valor del Tag UBL debe tener por lo menos uno -->	
<!-- 		<xsl:if -->
<!-- 			test="not(($codigoTipoMonto[text()='1001' or text()='1002' or text()='1003' or text()='1004' or text()='3001']))">			 -->
<!-- 			<xsl:call-template name="rejectCall"> -->
...
<!-- 		</xsl:if> -->
		...
		<!-- 37. No deberian de haber duplicados -->
		<xsl:if test="(count($codigoTipoMonto[text()='1001'])>1 or
				count($codigoTipoMonto[text()='1002'])>1 or 
				count($codigoTipoMonto[text()='1003'])>1 or 
				count($codigoTipoMonto[text()='1004'])>1 or 
		...
		<xsl:for-each select="$additionalMonetaryTotal">		
			...
			<xsl:if test="not(($codigoTipoMontoFor = '2001') or ($codigoTipoMontoFor = '2003'))">
			...
			<!-- 37. Total valor de venta - operaciones gravadas  -->
			<xsl:if
			...
			<xsl:if test="($codigoTipoMontoFor='1001')">								
				...
				<xsl:if test="($countAfectacionIGVLinea10=0) and ($countAfectacionIGVLinea17=0)">
					<xsl:call-template name="addWarning">
						...
						<xsl:with-param name="warningMessage" 
							select="concat('Error en la linea: a ', position(), ' ', $codigoTipoMontoFor, ' ', $countAfectacionIGVLinea10, ' ', $countAfectacionIGVLinea17)"/>
					</xsl:call-template>
				</xsl:if>
				...
				<xsl:if test="($countAfectacionIGVLinea10 &gt; 0) and ($totalValorVentaOperaciones=0)">
					<xsl:call-template name="addWarning">
						<xsl:with-param name="warningCode" select="'4016'" />
						<xsl:with-param name="warningMessage" 
							select="concat('Error en la linea: b ', position(), ' ', $codigoTipoMontoFor, ' ', $countAfectacionIGVLinea10, ' ', $totalValorVentaOperaciones)"/>
					</xsl:call-template>
				</xsl:if>								
			</xsl:if>
			
			<!-- 38. Total valor de venta - operaciones inafectas  -->
			...
			<!-- 39. Total valor de venta - operaciones exoneradas  -->
			<xsl:if test="($codigoTipoMontoFor='1003')">
				<xsl:if test="($countAfectacionIGVLinea20 &gt; 0) and ($totalValorVentaOperaciones=0)">
					<xsl:call-template name="addWarning">
						...
						<xsl:with-param name="warningMessage" 
							select="concat('Error en la linea: ', position(), ' ', $codigoTipoMontoFor, ' ',$countAfectacionIGVLinea20, ' ', $totalValorVentaOperaciones)"/>
					</xsl:call-template>
				</xsl:if>
		...
		<xsl:for-each select="cac:PrepaidPayment">
			<!-- 49. Serie y Número de documento que se realizo el anticipo -->
			<xsl:variable name="serieNumeroDocumentoRealizoAnticipo" select="./cbc:ID" />
			<!-- 49. Tipo de comprobante que se realizo el anticipo -->
		...
		<xsl:if test="($countTipoComproRealizoAnticipado02 &gt; 0) and ($totalAnticipos!=$sumaMontoAnticipado)" >
			<xsl:call-template name="addWarning">
				<xsl:with-param name="warningCode" select="'2509'" />
				<xsl:with-param name="warningMessage"
					select="concat('Error en la linea: ', $sumaMontoAnticipado, ' - ', $totalAnticipos, ' - ', $countTipoComproRealizoAnticipado02)"/>
			</xsl:call-template>
		</xsl:if>
					
		<xsl:if test="($countTipoComproRealizoAnticipado02=0) and ($totalAnticipos!=0)" >
			<xsl:call-template name="addWarning">
				<xsl:with-param name="warningCode" select="'2508'" />
				<xsl:with-param name="warningMessage"
					select="concat('Error en la linea: ',  $sumaMontoAnticipado, ' - ', $totalAnticipos, ' - ', $countTipoComproRealizoAnticipado02)"/>
			</xsl:call-template>
		</xsl:if>

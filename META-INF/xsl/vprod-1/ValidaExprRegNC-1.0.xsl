...
<!-- FIN: AXTEROID -->	
	...
	<xsl:template match="/*">
		...
		<xsl:variable name="codigoTipoNotaCredito" select="$serieNumeroDocumentoAfectado/cbc:ResponseCode" />	
		<!-- 7. Tipo de moneda -->
		...
		<xsl:if test="($codigoTipoNotaCredito!='10')">
		...
		<xsl:if test="($codigoTipoNotaCredito='10')">
				<xsl:call-template name="regexpValidateElementIfExist">
					...
					<xsl:with-param name="node" select="$serieNumeroDocumentoAfectadoID" />
					<xsl:with-param name="regexp" select="'^[F][A-Z0-9]{3}-[0-9]{1,8}$|^(E001)-[0-9]{1,8}$|^[B][A-Z0-9]{3}-[0-9]{1,8}$|^(EB01)-[0-9]{1,8}$|^[S][A-Z0-9]{3}-[0-9]{1,8}$|^[0-9]{1,4}-[0-9]{1,8}$'" />
					<xsl:with-param name="isError" select="false()"/> 
					<xsl:with-param name="descripcion" 
						select="concat('Error en la linea: ', $serieNumeroDocumentoAfectadoID, ' ', $cbcID)"/>
				</xsl:call-template>
		...
		<xsl:variable name="countAfectacionIGVLinea40" select="count(cac:CreditNoteLine/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cbc:TaxExemptionReasonCode[text()='40'])" />
		<xsl:if test="($countAfectacionIGVLinea40>0) or (substring($cbcID, 1, 1)='B')">
...
<!-- 			</xsl:call-template>  -->
		...
		<xsl:for-each select="cac:BillingReference">
			...
			<xsl:if
				...
				<xsl:call-template name="rejectCall">
					<xsl:with-param name="errorCode" select="'2116'" />
					<xsl:with-param name="errorMessage"
			...
			<xsl:if
				test="(substring($cbcID, 1, 1)='B') and ($tipoDocumentoDelDocumentoModifica!='03')">
				<xsl:call-template name="rejectCall">
					<xsl:with-param name="errorCode" select="'2399'" />
					<xsl:with-param name="errorMessage"
			...
			<xsl:if
				test="(substring($cbcID, 1, 1)='S') and ($tipoDocumentoDelDocumentoModifica!='14')">
				<xsl:call-template name="rejectCall">
					<xsl:with-param name="errorCode" select="'2991'" />
					<xsl:with-param name="errorMessage"
						select="concat('Error en la linea: ', $cbcID, ' ', $tipoDocumentoDelDocumentoModifica)"/>
				</xsl:call-template>
			</xsl:if>
...
<!-- 				<xsl:with-param name="regexp" select="'^[T][0-9]{3}-[0-9]{1,8}$|^[0-9]{4}-[0-9]{1,8}$|^[EG][0-9]{2}-[0-9]{1,8}$|^[G][0-9]{3}-[0-9]{1,8}$'"/> -->
		...
		<xsl:variable name="countTipoOtroDocumentoRelacionado99" select="count(cac:AdditionalDocumentReference/cbc:DocumentTypeCode[text()='99'])" />
		<xsl:for-each select="cac:AdditionalDocumentReference">	
			...
			<!-- 18. Descripción de tipo de otro documento relacionado -->
			<xsl:variable name="descripcionTipoOtroDocumentoRelacionado" select="./cbc:DocumentType"/>	
...
<!-- 				<xsl:with-param name="regexp" select="'[\s].{6,30}'"/> -->
			...
			<xsl:if
				...
				<xsl:call-template name="rejectCall">
					<xsl:with-param name="errorCode" select="'4009'" />
					<xsl:with-param name="errorMessage"
...
<!-- 				</xsl:call-template> -->
			...
			<xsl:if test="($codigoTipoNotaCredito!='10') and ($tipoOtroDocumentoRelacionado='99')">
				<!-- 18. Número de otro documento relacionado -->
				<xsl:call-template name="existValidateElementIfExist">
					<xsl:with-param name="errorCodeNotExist" select="'2636'"/>
					<xsl:with-param name="node" select="$numeroOtroDocumentoRelacionado"/>
					<xsl:with-param name="isError" select="false()"/> 
					<xsl:with-param name="descripcion" 
						select="concat('Error en la linea: ', position(), ' ', $numeroOtroDocumentoRelacionado)"/>
				</xsl:call-template>
			</xsl:if>	
			...
			<xsl:if test="($codigoTipoNotaCredito='10') and ($tipoOtroDocumentoRelacionado='99')">
				<!-- 18. Descripción de tipo de otro documento relacionado -->
				<xsl:call-template name="existValidateElementIfExist">
					<xsl:with-param name="errorCodeNotExist" select="'2637'"/>
					<xsl:with-param name="node" select="$descripcionTipoOtroDocumentoRelacionado"/>
					<xsl:with-param name="isError" select="false()"/> 
					<xsl:with-param name="descripcion" 
						select="concat('Error en la linea: ', position(), ' ', $descripcionTipoOtroDocumentoRelacionado)"/>
				</xsl:call-template>
			</xsl:if>	

			<xsl:if test="($codigoTipoNotaCredito='10') and ($countTipoOtroDocumentoRelacionado99>1)">
				<xsl:call-template name="addWarning">
					<xsl:with-param name="warningCode" select="'2635'" />
					<xsl:with-param name="warningMessage"
						select="concat('Error en la linea: ', $countTipoOtroDocumentoRelacionado99)"/>
				</xsl:call-template>
			</xsl:if>
		</xsl:for-each>
...
<!-- 			<xsl:with-param name="regexp" select="'^(?!\s*$).{1,250}'" /> -->
		...
		<xsl:for-each select="cac:CreditNoteLine">
			...
			<xsl:variable name="unidadMedidaItem" select="./cbc:CreditedQuantity/@unitCode"/>	
			<!-- 22. Cantidad de unidades por ítem -->	
			<xsl:variable name="cantidadUnidadesItem" select="./cbc:CreditedQuantity"/>
			<!-- 26. Valor unitario por ítem -->		
			<xsl:variable name="valorUnitarioItem" select="./cac:Price/cbc:PriceAmount"/>							
			<!-- 31. Valor de venta por línea -->
			...
			<xsl:variable name="countAfectacionIGVLinea10For" select="count(./cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cbc:TaxExemptionReasonCode[text()='10'])"/>	
			<xsl:variable name="countAfectacionIGVLinea20For" select="count(./cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cbc:TaxExemptionReasonCode[text()='20'])"/>	
			<xsl:variable name="countAfectacionIGVLinea30For" select="count(./cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cbc:TaxExemptionReasonCode[text()='30'])"/>				
			<xsl:for-each select="./cac:PricingReference/cac:AlternativeConditionPrice">
				...
				<!-- 28. Valor referencial unitario por ítem en operaciones no onerosas -->
				<xsl:if
					test="(($countAfectacionIGVLinea10For>0) or ($countAfectacionIGVLinea20For>0) or ($countAfectacionIGVLinea30For>0))">
					<xsl:if	test="($codigoPrecio='02') and ($precioVentaUnitarioItem>0)">	
						<xsl:call-template name="rejectCall">
			...
			<xsl:for-each select="./cac:TaxTotal">
				...
				<xsl:if test="($codigoTributoLinea='1000') and ($nombreTributoLinea!='IGV')">	
					<xsl:call-template name="addWarning">
						<xsl:with-param name="warningCode" select="'2377'" />
						<xsl:with-param name="warningMessage"
...
<!--  				<xsl:with-param name="regexp" select="'(-?[0-9]+){1,12}(\.[0-9]{1,2})?$'"/>  -->
		...
		<!-- 34. No deberian de haber duplicados -->
		<xsl:if
		...
		<xsl:for-each select="$additionalMonetaryTotal">		
			...
			<xsl:if test="not(($codigoTipoMontoFor='1001') or ($codigoTipoMontoFor='1002') or ($codigoTipoMontoFor='1003') or
				($codigoTipoMontoFor='1004') or ($codigoTipoMontoFor='2001') or ($codigoTipoMontoFor='2005'))">
				<xsl:call-template name="rejectCall">
					<xsl:with-param name="errorCode" select="'2150'" />
					<xsl:with-param name="errorMessage"
		...
		<xsl:if test="($totalAnticipos)">
			<xsl:if test="($totalAnticipos &lt;=0)">
				<xsl:call-template name="addWarning">
					<xsl:with-param name="warningCode" select="'2527'" />
					<xsl:with-param name="warningMessage"
						select="concat('Error en la linea: ', position(), ' ', $totalAnticipos)"/>
				</xsl:call-template>
			</xsl:if>
		</xsl:if>

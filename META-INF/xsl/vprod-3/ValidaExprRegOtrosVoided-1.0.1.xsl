...
<!-- FIN: AXTEROID -->	
	...
	<xsl:template match="/*">
		...
		<xsl:if test="substring-before($nombreArchivoEnviado,'.') != concat($rucFilename, '-', $cbcID)">
...
<!-- 				<xsl:with-param name="regexp" select="'^[0-9]{1,}?$'"/> -->
			...
			<!-- 10. Tipo de Documento -->			
			<xsl:choose>
				<xsl:when test="not(string($tipoDocumento))">
					<xsl:call-template name="rejectCall">
				...
				<xsl:otherwise>
					<xsl:if
						test="not($tipoDocumento = '20' or $tipoDocumento = '40')">
						<xsl:call-template name="rejectCall">
			...
			<xsl:if test="$tipoDocumento = '20' ">
				<xsl:call-template name="regexpValidateElementIfExist">
					<xsl:with-param name="errorCodeValidate" select="'2674'"/>
					<xsl:with-param name="node" select="$serieDocumentoDadoBaja"/>
					<xsl:with-param name="regexp" select="'^[R][A-Z0-9]{3}$|^[0-9]{1,4}$'"/>
					<xsl:with-param name="descripcion" 
						select="concat('Error en la linea: ', position(), ' ', $serieDocumentoDadoBaja)"/>
				</xsl:call-template>
			</xsl:if>
			
			<xsl:if test="$tipoDocumento = '40' ">
				<xsl:call-template name="regexpValidateElementIfExist">
					<xsl:with-param name="errorCodeValidate" select="'2675'"/>
					<xsl:with-param name="node" select="$serieDocumentoDadoBaja"/>
					<xsl:with-param name="regexp" select="'^[P][A-Z0-9]{3}$|^[0-9]{1,4}$'"/>
					<xsl:with-param name="descripcion" 
						select="concat('Error en la linea: ', position(), ' ', $serieDocumentoDadoBaja)"/>
				</xsl:call-template>
			</xsl:if>				
			...
			<!--13. Numero correlativo del documento de fin dentro de la serie --> 
			<xsl:choose>

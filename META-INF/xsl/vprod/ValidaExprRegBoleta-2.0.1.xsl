...
<!-- 					<xsl:with-param name="regexp" select="'^[T][A-Z0-9]{3}-[0-9]{1,8}$|^[0-9]{4}-[0-9]{1,8}$|^[EG]{2}[0-9]{2}-[0-9]{1,8}$|^[G]{1}[0-9]{3}-[0-9]{1,8}$'" /> -->
		...
		<xsl:variable name="porcentajeIGVVigenteLinea" select="sum(cac:InvoiceLine/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory[cac:TaxScheme/cbc:ID[text() = '1000' or text() = '9996']]/cbc:Percent)"/>
						...
						<xsl:for-each select="./cac:TaxScheme">
							...
							<xsl:if test="not($tipoOperacion = '0113')"> 
								<xsl:if test="((($codigoTributoLinea = 1000) and ($montoBase > 0)) or 
									...
									<xsl:if test="not($tasaIGVoIVAP = $porcentajeIGVVigenteLinea)">	
										<xsl:call-template name="rejectCall">
...
<!-- 										</xsl:call-template>																																														 -->
							...
							<xsl:if test="($codigoTributoLinea = 1000) ">						
								<xsl:if test="not($tasaIGVoIVAP = $porcentajeIGVVigente)">	
									<xsl:call-template name="addWarning">
	    ...
	    <xsl:if test="(cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '1000']]) and 
			...
			<xsl:if test="($dif_TotalImporte &lt; -1) or ($dif_TotalImporte &gt; 1)">
				<xsl:call-template name="addWarning">
					...
					<xsl:with-param name="warningMessage"
						select="concat('Error en la linea 1): ', $dif_TotalImporte, ' ', $sumatoriaIGV, ' ', $totalImporteCalculado, ' ', $totalBaseIGVxLinea, ' ', $sumatoriaDescuentosGlobales, ' ', $sumatoriaCargosGobales_49, ' ', $sumatoriaAnticipos_20, ' ',$porcentajeIGV)"/>
				</xsl:call-template>
			</xsl:if>
	    </xsl:if>
	    ...
	    <xsl:if test="(cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '1000']]) and 
			...
			<xsl:if test="($dif_TotalImporte &lt; -1) or ($dif_TotalImporte &gt; 1)">
				<xsl:call-template name="addWarning">
					<xsl:with-param name="warningCode" select="'4290'" />
					<xsl:with-param name="warningMessage"
						select="concat('Error en la linea 2): ', $dif_TotalImporte, ' ', $sumatoriaIGV, ' ', $totalImporteCalculado, ' ', $totalBaseIGVxLinea, ' ', $sumatoriaDescuentosGlobales, ' ', $sumatoriaCargosGobales_49, ' ', $porcentajeIGV)"/>
				</xsl:call-template>
			</xsl:if>
	    </xsl:if>
			...
			<xsl:if test="($sumatoriaTotalPrecioVentaSubTotal) and ($countCodigoTributo1016 = 0)">	
				...
				<xsl:if test="($dif_TotalImporte &lt; -1) or ($dif_TotalImporte &gt; 1)">
					<xsl:call-template name="addWarning">
						...
						<xsl:with-param name="warningMessage"
							select="concat('Error en la linea: a) ', $dif_TotalImporte, ' | ', $sumatoriaTotalPrecioVentaSubTotal, ' | ', $totalImporteCalculado, ' | ', $totalImpuestoCalculado, ' | ', $sumaTotalPrecioVenta, ' | ',$totalBaseIGVxLinea, ' | ',$sumatoriaDescuentosGlobales_02, ' | ',$sumatoriaCargosGobales_49, ' | ',$porcentajeIGV)"/>
					</xsl:call-template>
				</xsl:if>
			</xsl:if>	
		</xsl:if>   		 

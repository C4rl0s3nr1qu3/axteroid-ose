	    ...
	    <xsl:if test="cac:TaxTotal/cac:TaxSubtotal[cac:TaxCategory/cac:TaxScheme/cbc:ID[text() = '1000']]">
			...
			<xsl:if test="($dif_TotalImporte &lt; -1) or ($dif_TotalImporte &gt; 1)">
				<xsl:call-template name="rejectCall">
					...
					<xsl:with-param name="errorMessage"
						select="concat('Error en la linea: ', $dif_TotalImporte, ' ', $sumatoriaIGV, ' ', $totalImporteCalculado, ' ', $totalBaseIGVxLinea, ' ', $sumatoriaDescuentosGlobales, ' ', $sumatoriaCargosGobales_49, ' ',$sumatoriaAnticipos_20, ' ',$porcentajeIGV)"/>
				</xsl:call-template>
			</xsl:if>
	    </xsl:if>
		...
		<xsl:if test="($sumatoriaTotalPrecioVentaSubTotal) and ($countCodigoTributo1016 = 0)">	
			...
			<xsl:if test="($dif_TotalImporte &lt; -1) or ($dif_TotalImporte &gt; 1)">
				<xsl:call-template name="rejectCall">
					...
					<xsl:with-param name="errorMessage"
						select="concat('Error en la linea: a) ', $dif_TotalImporte, ' ', $sumatoriaTotalPrecioVentaSubTotal, ' ', $totalImporteCalculado, ' ', $totalImpuestoCalculado, ' ', $sumaTotalPrecioVenta, ' ', $porcentajeIGV)"/>
				</xsl:call-template>			
			</xsl:if>			
		</xsl:if>	   		 

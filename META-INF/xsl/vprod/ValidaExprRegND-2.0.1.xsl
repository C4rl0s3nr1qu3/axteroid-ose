		...
		<xsl:if test="($tipoDocumentoDocumentoModificaGlobal = '01') and
			...
			<xsl:if test="($dif_TotalImporte &lt; -1) or ($dif_TotalImporte &gt; 1)">
				<xsl:call-template name="rejectCall">
					...
					<xsl:with-param name="errorMessage"
						select="concat('Error en la linea: ', $dif_TotalImporte, ' ', $sumatoriaIGV, ' ', $totalImporteCalculado, ' ', $totalBaseIGVxLinea, ' ', $porcentajeIGV)"/>
				</xsl:call-template>				
			</xsl:if>	
		</xsl:if>	
...
<!-- 				</xsl:call-template> -->
		...
		<xsl:variable name="porcentajeIGVVigenteLinea" select="sum(cac:DebitNoteLine/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory[cac:TaxScheme/cbc:ID[text() = '1000' or text() = '9996']]/cbc:Percent)"/>
						...
						<xsl:for-each select="./cac:TaxScheme">						
							...
							<xsl:if test="($tipoDocumentoDocumentoModificaGlobal = '01')">
								<xsl:if test="((($codigoTributoLinea = 1000) and ($montoBase > 0)) or 
									...
									<xsl:if test="not($tasaIGV = $porcentajeIGVVigenteLinea)">	
										<xsl:call-template name="rejectCall">
...
<!--										</xsl:call-template>																																														-->
							...
							<xsl:if test="($codigoTributoLinea = 1000) ">						
								<xsl:if test="not($tasaIGV = $porcentajeIGVVigente)">	
									<xsl:call-template name="addWarning">

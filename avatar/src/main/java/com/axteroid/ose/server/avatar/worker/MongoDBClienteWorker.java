...
public class MongoDBClienteWorker {
    static Logger log = LoggerFactory.getLogger(MongoDBClienteWorker.class);
    ...
    private void send2OseSever(String urlOSE, String ip, String usuario, String clave, String contar, String error_numero, 
    	...
    	log.info("fecJBossInicio: "+ fecJBossInicio+" fecJBossFin"+ fecJBossFin 
    			+" | fecDBInicio "+fecDBInicio+ " fecDBFin "+fecDBFin);
    	try {       	
    		SendOseSeverBuild sendOseSeverBuild = new SendOseSeverBuild(); 
        	String estado = "20";           	
    		if((proceso==0) || (proceso==1))
    			sendOseSeverBuild.sendOseSeverDia(urlOSE, ip, usuario, clave, contar, error_numero, estado, 
    					proceso, fecDBInicio, fecDBFin);
    		...
    		if((proceso==80) || (proceso==81)|| (proceso==90) || (proceso==91) || (proceso==100) || (proceso==101)) {
    			...
    			sendOseSeverBuild.sendOseSeverDiaELBD(urlOSE, ip, usuario, clave, contar, error_numero, estado, 
    				proceso, fecJBossInicio, fecJBossFin, fecDBInicio, fecDBFin, script, ruc); 
    			...
    			sendOseSeverBuild.sendOseSeverDiaJoinEL(urlOSE, ip, usuario, clave, contar, error_numero, estado, 
    				procesoNuevo, fecJBossInicio, fecJBossFin, fecDBInicio, fecDBFin, script, ruc);
    		}    		
	    } catch (Exception e) {
			StringWriter errors = new StringWriter();				
			e.printStackTrace(new PrintWriter(errors));
			log.error("send2OseServer Exception \n"+errors);
		} 
    }
}

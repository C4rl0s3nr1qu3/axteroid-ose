...
public class ShipmentType {
	...
	public void insertTransportModeType(TransportModeCodeType value) {
		if(shipmentStage.size() == 0) {
			ShipmentStageType shipmentStageType = new ShipmentStageType();		
			shipmentStageType.setTransportModeCode(value);
			getShipmentStage().add(shipmentStageType);			
		} else {
			getShipmentStage().get(0).setTransportModeCode(value);
		}
	}
	
	public void insertStartDateType(StartDateType value) {
		PeriodType periodType = new PeriodType();
		periodType.setStartDate(value);
		...
		if(shipmentStage.size() == 0) {
			...
			getShipmentStage().get(0).setTransitPeriod(periodType);
		}
	}
	
	public void insertPartyIdentificationType(PartyType value) {
		if(shipmentStage.size() == 0) {
			ShipmentStageType shipmentStageType = new ShipmentStageType();
			shipmentStageType.getCarrierParty().add(value);
			getShipmentStage().add(shipmentStageType);
		} else {
			getShipmentStage().get(0).getCarrierParty().add(value);
		}
	}
	
	public void insertPartyNameType(PartyNameType value){
        if(value == null) return;
        
//      PartyNameType partyNameType = new PartyNameType();
//      partyNameType.setName(value);
//      PartyType partyType = new PartyType();
//      partyType.setPartyName(Arrays.asList(value));
...
//            getShipmentStage().add(shipmentStageType);
//      } else {
              getShipmentStage().get(0).getCarrierParty().get(0).getPartyName().add(value);
//      }

	}
	
	public void insertDriverPersonType(PersonType value) {
		if(shipmentStage.size() == 0) {
			ShipmentStageType shipmentStageType = new ShipmentStageType();
			shipmentStageType.setDriverPerson(value);
			getShipmentStage().add(shipmentStageType);
		} else {
			getShipmentStage().get(0).setDriverPerson(value);
		}
	}
	
	public void insertTransportHandlingUnitType(TransportHandlingUnitType value) {
		getTransportHandlingUnit().add(value);
	}
	
	public void insertRoadTransportType(RoadTransportType value) {
		if(value == null) return;
		TransportMeansType tmType = new TransportMeansType();
		tmType.setRoadTransport(value);
		getShipmentStage().get(0).setTransportMeans(tmType);
	}
	
//	public void insertDeliveryAddress(AddressType value){
//		if(value == null) return;
//		getDelivery().setDeliveryAddress(value);
